//this file is for PS accumulation 
...
void template_PS(){
	...
	chain->Add("/home/s2022220206/Final_exam/file/FADC_data_2.root");
	...
	TH1D * PS_accu = new TH1D("PS_accu","PS_accu",40,0,40); // pdf
	...
	for (int j = 0 ; j < 40 ; j ++){
		...
		PS_accu->SetBinContent(j+1,sum);
	}
	PS_accu->Scale(1.0/PS_accu->Integral());

	for (int i = 0 ; i < 40 ; i++) temp[i] = PS_accu->GetBinContent(i+1);
...
//		cout << conv_list[i] << endl;
	...
	// skimming
	for (int i = 0 ; i < total ; i++){
		if ((conv_list[i] < 0.06) || (Qtotal_list[i] < 1200)){
//			cout << "max convolution value : " <<	conv_list[i] << " , Qtotal : " << Qtotal_list[i] << ", event number : " <<  i << endl;
	...
	TFile o("/home/s2022220206/Final_exam/PS/PS_accu.root","RECREATE");
	PS_accu->Write("hist");
